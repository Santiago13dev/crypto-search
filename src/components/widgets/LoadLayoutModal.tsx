              ...
              <div className="p-6 overflow-y-auto max-h-[calc(80vh-140px)] scrollbar-terminal">
                {savedLayouts.length === 0 ? (
                  <div className="text-center py-12">
                    <ArrowPathIcon className="w-16 h-16 text-primary/30 mx-auto mb-4" />
                      ...
                      <motion.div
                        ...
                        className={`p-4 rounded-lg cursor-pointer transition-all ${
                          activeLayoutId === layout.id
                            ? 'bg-[#00ff00]/20 border-2 border-primary'
                            : 'bg-[#00ff00]/5 border border-primary/30 hover:border-primary hover:bg-[#00ff00]/10'
                        }`}
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-xs text-primary/60 font-mono">
                            ...
                            <div className="mt-2 flex flex-wrap gap-1">
                              {[...new Set(layout.widgets.map(w => w.type))].map((type) => (
                                <span
                                  key={type}
                                  className="text-xs px-2 py-1 bg-[#00ff00]/10 border border-primary/30 rounded font-mono"
                                >
                                  {type}
                                </span>
                              ))}
                          ...
                          >
                            <TrashIcon className="w-4 h-4 text-primary group-hover:text-red-500" />
                          </button>
                        </div>
                      </motion.div>
                    ))}
