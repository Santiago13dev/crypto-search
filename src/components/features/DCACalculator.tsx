...
export default function DCACalculator() {
  ...
  return (
    <div className="p-6 border border-[#00ff00]/20 bg-[#0a0f1e] rounded-none">
      <div className="mb-6">
        <h3 className="text-xl font-bold text-[#00ff00] font-mono mb-2">{`>`} Dollar Cost Averaging (DCA)</h3>
        <p className="text-sm text-[#00ff00]/60 font-mono">Calcula el rendimiento de inversiones periódicas</p>
      </div>

      <form onSubmit={handleCalculate} className="space-y-6">
        <div>
          <label className="block text-sm text-[#00ff00]/80 font-mono mb-2">{`>`} Criptomoneda</label>
          {coin ? (
            <div className="flex items-center gap-3 p-3 border border-[#00ff00]/20 rounded-none">
              ...
              <button type="button" onClick={() => setCoin(null)} className="ml-auto text-[#00ff00]/60 hover:text-[#00ff00] text-sm font-mono underline">Cambiar</button>
            </div>
          ) : (
            <div className="relative">
              <input type="text" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} placeholder="Buscar criptomoneda..." className="w-full bg-[#0a0f1e] border border-[#00ff00]/40 text-[#00ff00] px-4 py-3 font-mono focus:outline-none rounded-none" />
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#0a0f1e] border border-[#00ff00]/40 rounded-none max-h-60 overflow-y-auto z-50">
                  {searchResults.slice(0, 5).map((c) => (
                    <button key={c.id} type="button" onClick={() => { setCoin(c); setSearchQuery(''); setSearchResults([]); }} className="w-full p-3 hover:bg-[#00ff00]/10 flex items-center gap-3 text-left">
                      ...
                      <span className="font-mono text-[#00ff00]">{c.name}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div>
          <label className="block text-sm text-[#00ff00]/80 font-mono mb-2">{`>`} Monto por compra (USD)</label>
          <input type="number" step="any" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-[#0a0f1e] border border-[#00ff00]/40 text-[#00ff00] px-4 py-3 font-mono focus:outline-none rounded-none" required />
        </div>

        <div>
          <label className="block text-sm text-[#00ff00]/80 font-mono mb-2">{`>`} Frecuencia</label>
          <div className="grid grid-cols-3 gap-2">
            {[{ value: 'daily' as const, label: 'Diario' }, { value: 'weekly' as const, label: 'Semanal' }, { value: 'monthly' as const, label: 'Mensual' }].map((freq) => (
              <button key={freq.value} type="button" onClick={() => setFrequency(freq.value)} className={`px-4 py-2 font-mono text-sm transition-all rounded-none ${frequency === freq.value ? 'bg-[#00ff00] text-black font-bold' : 'bg-[#0a0f1e] border border-[#00ff00]/20 text-[#00ff00]'}`}>{freq.label}</button>
            ))}
          </div>
        </div>

        <div>
          <label className="block text-sm text-[#00ff00]/80 font-mono mb-2">{`>`} Duración (días)</label>
          <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full bg-[#0a0f1e] border border-[#00ff00]/40 text-[#00ff00] px-4 py-3 font-mono focus:outline-none rounded-none" required />
        </div>
        ...
        {result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-4 mt-6">
            <div className="p-4 border border-[#00ff00]/20 rounded-none">
              <p className="text-xs text-[#00ff00]/60 font-mono mb-1">Total Invertido</p>
              <p className="text-2xl font-bold text-[#00ff00] font-mono">${result.totalInvested.toLocaleString()}</p>
            </div>
            <div className="p-4 border border-[#00ff00]/20 rounded-none">
              <p className="text-xs text-[#00ff00]/60 font-mono mb-1">Valor Actual</p>
              <p className="text-2xl font-bold text-blue-400 font-mono">${result.currentValue.toLocaleString()}</p>
            </div>
            <div className="p-4 border border-[#00ff00]/20 rounded-none">
              <p className="text-xs text-[#00ff00]/60 font-mono mb-1">Ganancia/Pérdida</p>
            ...
            <div className="p-4 border border-[#00ff00]/20 rounded-none">
              <p className="text-xs text-[#00ff00]/60 font-mono mb-1">Total Monedas</p>
              <p className="text-2xl font-bold text-[#00ff00] font-mono">{result.totalCoins.toFixed(6)}</p>
            </div>
          </motion.div>
        )}
      </form>
